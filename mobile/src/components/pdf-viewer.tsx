...
function updateUI() {
  ...
  sendMsg('page', { current: currentPage, total: totalPages });
}
...
export function PdfViewer({ url, onPageChange }: PdfViewerProps) {
  ...
  const html = useMemo(() => generatePdfHtml(url), [url]);
  ...
  const handleMessage = useCallback(
    (event: WebViewMessageEvent) => {
      try {
        ...
        if (data.type === 'error') {
          setError(data.message || t('viewer.loadFailed'));
        } else if (data.type === 'page' && typeof data.current === 'number') {
          onPageChange?.({ pageIndex: data.current - 1, pageCount: data.total });
        }
      } catch {
