...
func RegisterRoutes(h *Handler, authHandler *AuthHandler, mux *http.ServeMux, jwtMiddleware *middleware.JWTMiddleware, authzEnforcer *casbin.Enforcer) {
	...
	handle("POST /api/v1/documents/suggest-folder", h.SuggestFolder, middleware.Authorize(authzEnforcer, authz.ResourceDocuments, authz.ActionRead))
	...
	handle("GET /api/v1/documents/{id}/progress", h.DocumentProgressSSE)
