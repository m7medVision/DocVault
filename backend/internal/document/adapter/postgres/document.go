// Package postgres is the document bounded context's data-access adapter. It
// wraps the shared sqlc Queries for documents, folders, tags, full-text/vector
// search, and the ACL visibility seam, mapping rows to the document domain
// model.
package postgres
...
func NewDocumentRepository(db *pgxpool.Pool) repository.DocumentRepository {
...
func (r *documentRepository) List(ctx context.Context, q *repository.ListDocumentsQuery) ([]model.Document, *string, error) {
	if q.Limit <= 0 {
		q.Limit = 20
	}
...
func (r *documentRepository) Update(ctx context.Context, doc *model.Document) error {
	...
	if err != nil {
		if isTitleConflict(err) {
			return repository.ErrDocumentTitleExists
		}
