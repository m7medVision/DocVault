...
func (s *stubQueuePublisher) Publish(_ context.Context, body []byte) error {
...
func TestUploadPublishesOCRJobContract(t *testing.T) {
	...
	svc := service.NewDocumentService(repo, nil, publisher)

	output, err := svc.Upload(context.Background(), &service.UploadDocumentInput{
		TenantID: "tenant-1",
		OrgID:    "org-1",
		OwnerID:  "user-1",
		Title:    "Passport",
	...
	if err := json.Unmarshal(publisher.body, &job); err != nil {
...
var _ service.QueuePublisher = (*stubQueuePublisher)(nil)
